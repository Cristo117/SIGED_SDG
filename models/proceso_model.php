 ...
 */
function asegurarColumnaEstadoPagoProceso($conn) {
    try {
 ...
 */
function obtenerProcesosActivos($conn) {
 ...
 */
function agregarProceso($conn, $nombre, $valor) {
    $nombre = trim($nombre);
 ...
 */
function actualizarProceso($conn, $id, $nombre, $valor) {
    $id = (int) $id;
    $nombre = trim($nombre);
    $valor = max(0, (float) preg_replace('/[^\d]/', '', (string) $valor));
 ...
 */
function eliminarProceso($conn, $id) {
    $id = (int) $id;
    if ($id <= 0) {
 ...
 */
function obtenerClientesParaAsignar($conn) {
 ...
 */
function asignarProcesoACliente($conn, $clienteId, $procesoId) {
    asegurarColumnaEstadoPagoProceso($conn);
 ...
 */
function obtenerProcesosPorCliente($conn, $clienteId) {
    $clienteId = (int) $clienteId;
    $stmt = $conn->prepare("
 ...
 */
function obtenerNombreCliente($conn, $clienteId) {
 ...
 */
function obtenerEstadoPagoCliente($conn, $clienteId) {
    $clienteId = (int) $clienteId;
    $stmt = $conn->prepare("
 ...
 */
function toggleEstadoPagoProcesoCliente($conn, $procesoClienteId) {
    ...
    $estadoCliente = obtenerEstadoPagoCliente($conn, $clienteId);
...
function obtenerEstadisticasReportes($conn) {
    // Asegurar que el modelo de configuración esté cargado
    if (!function_exists('obtenerValorAfiliacion')) {
        require_once __DIR__ . '/configuracion_model.php';
    }
    $valorAfiliacion = obtenerValorAfiliacion($conn);
 ...
 */
function obtenerTopClientes($conn, $limite = 5) {
 ...
 */
function obtenerIngresosPorTipo($conn) {
    $stats = obtenerEstadisticasReportes($conn);
    $empleador = 0;
    $independiente = 0;
 ...
 */
function obtenerConfiguracionValores($conn) {
    // Asegurar que el modelo de configuración esté cargado
    if (!function_exists('obtenerValorAfiliacion')) {
        require_once __DIR__ . '/configuracion_model.php';
    }
    return [
        'valor_afiliacion' => obtenerValorAfiliacion($conn),
        'procesos' => obtenerProcesosActivos($conn)
    ];
}
 ...
 */
function contarProcesosActivos($conn) {
